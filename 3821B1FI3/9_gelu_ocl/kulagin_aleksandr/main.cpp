...
static int test_correctness() {
  ...
  std::vector<float> out1 = GeluSEQ(input), out2 = GeluOCL(input);
  assert(!out2.empty());
  for (std::vector<float>::size_type i = 0; i < sz; i++) {
...
static void test_time() {
  ...
  std::vector<float> input(sz);
  for (std::vector<float>::size_type i = 0; i < sz; i++) {
    input[i] = (float)(i % 10000) / M_PIf;
  }
  std::vector<float> input_pre(input);
  input_pre.resize(sz / 8);
  (void)GeluOCL(input_pre);

  auto start = std::chrono::high_resolution_clock::now();
  std::vector<float> out = GeluOCL(input);
  auto end = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double> duration = end - start;
  std::cout << "ELAPSED TIME OCL " << duration.count() << '\n';
