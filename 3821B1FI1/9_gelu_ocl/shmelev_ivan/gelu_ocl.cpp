...
__kernel void gelu_kernel(__global const float* x, __global float* y, int countElem) {
  ...
  CHECK_CL_ERROR(queue.enqueueWriteBuffer(bufferInput, CL_TRUE, 0, countBytes, input.data()));
  ...
  CHECK_CL_ERROR(queue.enqueueReadBuffer(bufferOutput, CL_TRUE, 0, countBytes, output.data()));
