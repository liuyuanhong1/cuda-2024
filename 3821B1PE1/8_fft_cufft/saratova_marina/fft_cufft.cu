...
__global__ void normalize_kernel(float* x, size_t size, float coef) {
  size_t i = static_cast<size_t>(blockDim.x) * static_cast<size_t>(blockIdx.x) + static_cast<size_t>(threadIdx.x);
  if (i < size) {
    x[i] *= coef;
  }
}
...
std::vector<float> FffCUFFT(const std::vector<float>& input, int batch) {
  ...
  CHECK_CUDA_ERROR(cudaMemcpy(reinterpret_cast<void*>(signal), reinterpret_cast<const void*>(input.data()), countBytes, cudaMemcpyHostToDevice));
  ...
  normalize_kernel<<<numBlocks, threadsPerBlock>>>(reinterpret_cast<float*>(signal), size, 1.0f / elemPerBatch);
  CHECK_CUDA_ERROR(cudaDeviceSynchronize());
  CHECK_CUDA_ERROR(cudaGetLastError());

  CHECK_CUDA_ERROR(cudaMemcpy(reinterpret_cast<void*>(output.data()), reinterpret_cast<void*>(signal), countBytes, cudaMemcpyDeviceToHost));
