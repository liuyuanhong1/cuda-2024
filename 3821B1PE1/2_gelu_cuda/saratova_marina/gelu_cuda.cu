...
std::vector<float> GeluCUDA(const std::vector<float> &input) {
  ...
  CHECK_CUDA_ERROR(cudaMemcpy(reinterpret_cast<void *>(inpDev), reinterpret_cast<const void *>(input.data()), countBytes, cudaMemcpyHostToDevice));

  gelu_kernel<<<numBlocks, threadsPerBlock>>>(outDev, inpDev, size);
  CHECK_CUDA_ERROR(cudaDeviceSynchronize());
  CHECK_CUDA_ERROR(cudaGetLastError());

  CHECK_CUDA_ERROR(cudaMemcpy(reinterpret_cast<void *>(output.data()), reinterpret_cast<void *>(outDev), countBytes, cudaMemcpyDeviceToHost));
