...
std::vector<float> NaiveGemmCUDA(const std::vector<float>& a, const std::vector<float>& b, int n) {
  ...
  cudaMemcpy(d_a, a.data(), static_cast<unsigned long long>(n) * n * sizeof(float), cudaMemcpyHostToDevice);
  cudaMemcpy(d_b, b.data(), static_cast<unsigned long long>(n) * n * sizeof(float), cudaMemcpyHostToDevice);
  dim3 threadsPerBlock(16, 16);
  dim3 numBlocks((n + threadsPerBlock.x - 1) / threadsPerBlock.x, (n + threadsPerBlock.y - 1) / threadsPerBlock.y);
  MatrixMulKernel<<<numBlocks, threadsPerBlock>>>(d_a, d_b, d_c, n);
  cudaMemcpy(c.data(), d_c, static_cast<unsigned long long>(n) * n * sizeof(float), cudaMemcpyDeviceToHost);
  cudaFree(d_a);
  cudaFree(d_b);
  cudaFree(d_c);
  return c;
